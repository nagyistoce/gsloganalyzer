##
## AccessLog filter for use in traditional Unix scripting
##
##        Author: tkack
##          Date: 2012-10-25 
...
BEGIN {
	...
	# to keep the position in the stream.
	position = 1 
	
	# Since we need to transform the date into a ISO8601 compliant format 
	# we need to create a month transformation associate array
	monthNumber["Jan"]="01"
	monthNumber["Feb"]="02"
	monthNumber["Mar"]="03"
	monthNumber["Apr"]="04"
...
# Create a string that can be safely exported
...
{
# $1 => POSITION, THREAD, IP, USERID,DATE,TIME,TZ, DATETIME
	delete request
	...
	if (x[4]=="{unset") {
		unsetOffset=1
		request["USERID"]=sprintf("%s %s",x[4],x[5])
	}  
	sub(/\[/,"",x[5+unsetOffset])
	sub(/\]/,"",x[6+unsetOffset])
	split(x[5+unsetOffset],dt,":")
	request["DATE"]=sprintf("%s",dt[1])
	request["TIME"]=sprintf("%s:%s:%s",dt[2],dt[3],dt[4])
	request["TZ"]=x[6+unsetOffset]
...
# TODO: There should only be one exporter format. Probably JSON and then there will be a json2csv converter etc.
# One problem with using JSON is that it is quite involved to convert to/from in Bash
# and it is quite verbose as well.
# Example of how JSON looks like
# {"Entry": {
#     "Ip": "1.2.3.4",
#     "window": {
